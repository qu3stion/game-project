...
function shuffledDeck(deck){
  ...
  // these lines represent taking the 52nd card and putting it in a random deck position
  // and then going down by one and applying that same operation to the next lowest card
  }
  return shuffledDeck;
}
console.log(shuffledDeck(buildDeck()));
...
function greet(){
  let name = getInput("Welcome to * Hi/Lo * What's your name?")
...
function guess(card1, card2){
  ...
  let input = getInput(`What do you thiink the next card will be? Will it be higher (h) or lower(l)?`)
...
function playGame(deck){
  let deck1 = shuffledDeck(buildDeck())
  ...
  while (score < 5 && score < deck.length) {
    ...
    if (guess(currentCard, nextCard) === true) {
      score++
      console.log(`Congrats! Correct guess; your score is now ${score}`);
    } else {
      console.log(`Sorry, you were incorrect and received no points.`);
    }
